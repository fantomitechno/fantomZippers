...
{
  [CustomEntity("EstebCollabFantom/MouseZipper", "fantomZippers/MouseZipper")]
  public class MouseZipper : Solid
  {
    public MouseZipper(Vector2 position, int width, int height, Vector2 target, Themes theme) : base(position, (float)width, (float)height, false)
    {
      if (theme == Themes.Moon)
      {
        this.spritePath = "fantomitechno/fantomZippers/mouse/light";
    ...
    public MouseZipper(EntityData data, Vector2 offset) : this(data.Position + offset, data.Width, data.Height, data.Nodes[0] + offset, data.Enum("theme", Themes.Normal))
    {
    }
    ...
    private IEnumerator Sequence()
    {
      ...
      {
        yield return null;
        int new_value = Mouse.GetState().ScrollWheelValue;

        float delta = ((float)(new_value - old_value)) / 6000;
        old_value = new_value;

        this.percent = Math.Max(Math.Min(this.percent + delta, 1), 0);
        if (percent < 0.33)
        {
    ...
    {
      public ZipMoverPathRenderer(MouseZipper zipMover, string spritePath) : base()
      {
